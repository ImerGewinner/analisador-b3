...
from fundamentals import FUNDAMENTALS_SOURCE, ensure_fundamentals_schema, load_and_score_fundamentals
...
SHEET_SEED_URL = (
    ...
    "1TSG_UF5FO0evEi2wRzi8nmYxKDUdyuOyMJvMayAm6yA/"
    "export?format=csv&gid=742583260"
)
...
def connect() -> sqlite3.Connection:
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS universe (
          ticker TEXT PRIMARY KEY, cnpj TEXT, cvm_code TEXT, root TEXT, company TEXT,
          trading_name TEXT, segment TEXT, listing_date TEXT, price REAL, variation REAL,
          volume_day REAL, avg20 REAL, avg60 REAL, sessions20 INTEGER, sessions60 INTEGER,
          stale_sessions INTEGER, principal TEXT, passes_liquidity TEXT,
          passes_activity TEXT, eligible TEXT, reason TEXT, quote_date TEXT,
          source TEXT, updated_at TEXT
        );
        CREATE TABLE IF NOT EXISTS history (
          quote_date TEXT NOT NULL, ticker TEXT NOT NULL, close REAL, volume REAL,
          trades INTEGER, PRIMARY KEY (quote_date,ticker)
        );
...
def seed_rows() -> list[dict]:
    if SEED_PATH.exists():
        ...
        return list(reader)
    response=requests.get(SHEET_SEED_URL,timeout=90)
    response.raise_for_status()
    raw=list(csv.reader(io.StringIO(response.text)))
    header_index=next((i for i,row in enumerate(raw) if row and row[0].strip()=="CNPJ"),None)
    if header_index is None:
        raise RuntimeError("Cabeçalho do Universo B3 não encontrado na planilha-base.")
    header=raw[header_index]
    result=[]
    for values in raw[header_index+1:]:
        if len(values)<6 or not values[5].strip():
            continue
...
def load_seed() -> tuple[dict[str,dict],dict[str,dict]]:
    ...
    for row in seed_rows():
        ...
        root=str(row.get("root") or row.get("Raiz") or "").upper().strip()
        if not ticker or not root:
            continue
        item={
            "ticker":ticker,
            "root":root,
            "cnpj":str(row.get("cnpj") or row.get("CNPJ") or "").strip(),
            "cvm_code":normalized_code(row.get("cvm_code") or row.get("Código CVM") or ""),
            "company":str(row.get("company") or row.get("Empresa") or "").strip(),
            "trading_name":str(row.get("trading_name") or row.get("Nome negociação") or "").strip(),
        }
        by_ticker[ticker]=item
        by_root[root]=item
    if not by_root:
        raise RuntimeError("Mapeamento CNPJ/ticker ficou vazio.")
...
def fetch_companies() -> dict[str,dict]:
    ...
    while page<=total_pages:
        body=b3_json(B3_COMPANIES_API,{"language":"pt-br","pageNumber":page,"pageSize":120})
        total_pages=int((body.get("page") or {}).get("totalPages") or total_pages)
        for row in body.get("results") or []:
            if str(row.get("status"))!="A" or str(row.get("type"))!="1" or str(row.get("typeBDR") or "").strip():
                continue
            root=str(row.get("issuingCompany") or "").upper().strip()
            if not root:
                continue
            score=(0 if str(row.get("marketIndicator"))=="99" else 4)+(0 if str(row.get("dateListing"))=="31/12/9999" else 2)
...
def fixed_number(value: str, decimals: int=2) -> float:
    digits=value.strip()
    return int(digits)/(10**decimals) if digits else 0.0
...
def parse_cotahist(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        ...
        if len(ticker)<5 or ticker[4:] not in {"3","4","5","6","7","8","11"}:
            continue
        ymd=line[2:10]
        result.append({
            "quote_date":f"{ymd[:4]}-{ymd[4:6]}-{ymd[6:8]}","ticker":ticker,
            "company":line[27:39].strip(),"close":fixed_number(line[108:121]),
            "trades":int(line[147:152].strip() or 0),"volume":fixed_number(line[170:188]),
        })
    return result


def fetch_cotahist(day: date) -> list[dict] | None:
    url=f"{B3_COTAHIST_BASE}COTAHIST_D{day.strftime('%d%m%Y')}.ZIP"
    response=requests.get(url,timeout=90,allow_redirects=True)
    if response.status_code==404:
        return None
    response.raise_for_status()
    with zipfile.ZipFile(io.BytesIO(response.content)) as archive:
        name=next((n for n in archive.namelist() if n.upper().endswith(".TXT")),archive.namelist()[0])
        return parse_cotahist(archive.read(name).decode("latin-1")) or None
...
def fetch_latest_cotahist(max_days: int=15) -> tuple[date,list[dict]]:
    for offset in range(max_days+1):
        ...
        if day.weekday()<5:
            ...
            if rows:
                return day,rows
    raise RuntimeError("Nenhum COTAHIST diário localizado.")
...
def save_history(conn: sqlite3.Connection, rows: list[dict]) -> None:
    conn.executemany(
        ...
        ON CONFLICT(quote_date,ticker) DO UPDATE SET close=excluded.close,volume=excluded.volume,trades=excluded.trades""",
        rows,
    )
    conn.commit()
...
def backfill_history(conn: sqlite3.Connection, target_days: int=65) -> None:
    ...
    cursor=date.today()
    attempts=0
    while history_days(conn)<target_days and attempts<120:
        if cursor.weekday()<5:
            try:
                rows=fetch_cotahist(cursor)
                if rows:
                    save_history(conn,rows)
                    print(f"Histórico: {history_days(conn)} pregões")
...
def average(values: list[float], denominator: int) -> float:
    return sum(values)/denominator if denominator else 0.0


def update_universe(conn: sqlite3.Connection, companies: dict[str,dict], quote_day: date, quotes: list[dict]) -> int:
    seed_by_ticker,seed_by_root=load_seed()
    previous={row["ticker"]:row["close"] for row in conn.execute(
        """SELECT h.ticker,h.close FROM history h JOIN(
        SELECT ticker,MAX(quote_date) quote_date FROM history WHERE quote_date<? GROUP BY ticker
        )x ON x.ticker=h.ticker AND x.quote_date=h.quote_date""",(quote_day.isoformat(),)
    )}
    ...
    for quote in quotes:
        ...
        if not company and not seed:
            continue
        hist=list(conn.execute("SELECT volume FROM history WHERE ticker=? ORDER BY quote_date DESC LIMIT 60",(ticker,)))
        volumes=[float(row["volume"] or 0) for row in reversed(hist)]
        last20,last60=volumes[-20:],volumes[-60:]
        stale=len(last60)
        for idx,value in enumerate(reversed(last60)):
            if value>0:
                stale=idx
                break
        avg20,avg60=average(last20,20),average(last60,60)
        sessions20,sessions60=sum(v>0 for v in last20),sum(v>0 for v in last60)
        passes_liquidity="SIM" if avg20>=FILTERS["liquidez_minima_20d"] else "NÃO"
        passes_activity="SIM" if sessions20>=FILTERS["pregoes_minimos_20d"] and stale<=FILTERS["max_sessoes_sem_negocio"] else "NÃO"
        eligible="SIM" if passes_liquidity==passes_activity=="SIM" else "NÃO"
        reason="Passou no filtro inicial" if eligible=="SIM" else ("Liquidez insuficiente" if passes_liquidity=="NÃO" else "Negociação pouco frequente")
        ...
        conn.execute(
            ...
            ON CONFLICT(ticker) DO UPDATE SET cnpj=excluded.cnpj,cvm_code=excluded.cvm_code,
            ...
            passes_activity=excluded.passes_activity,eligible=excluded.eligible,reason=excluded.reason,
            quote_date=excluded.quote_date,source=excluded.source,updated_at=excluded.updated_at""",
            (ticker,seed.get("cnpj",""),seed.get("cvm_code",""),root,
             company.get("company") or seed.get("company") or quote["company"],
             company.get("trading_name") or seed.get("trading_name") or "",
             company.get("segment") or "",company.get("listing_date") or "",quote["close"],variation,
             quote["volume"],avg20,avg60,sessions20,sessions60,stale,"NÃO",passes_liquidity,
             passes_activity,eligible,reason,quote_day.isoformat(),"B3 COTAHIST — fechamento oficial",now),
        )
        changed+=1
    for root, in conn.execute("SELECT DISTINCT root FROM universe"):
        rows=list(conn.execute("SELECT ticker,avg20 FROM universe WHERE root=? ORDER BY avg20 DESC,ticker",(root,)))
        if rows:
...
def fmt_money(value: float | None) -> str:
    return "" if value is None else f"R$ {value:,.2f}".replace(",","X").replace(".",",").replace("X",".")
...
def build_site(conn: sqlite3.Connection) -> dict:
    rows=list(conn.execute(
        """SELECT u.*,f.reference_date fundamentals_ref,f.origin fundamentals_origin,
        f.revenue_ltm,f.profit_ltm,f.ebitda_ltm,f.equity,f.cash,f.debt,f.net_debt,
        f.roe_avg_5y,f.profit_cagr_5y,f.net_margin_latest,f.margin_trend,
        f.positive_profits_5y,f.net_debt_ebitda,f.payout,f.is_financial,
        f.quality_status,f.quality_score,f.failures,f.pending,f.reason quality_reason,
        f.criteria_json,f.source_url fundamentals_source
    ...
    for row in rows:
        try:
            ...
            criteria=[]
        original=row["quality_status"] or "PENDENTE FUNDAMENTOS"
        display=original if row["eligible"]=="SIM" else "FORA DO UNIVERSO INICIAL"
        items.append({
            "ticker":row["ticker"],"empresa":row["company"],"segmento":row["segment"],
            "cnpj":row["cnpj"] or "","codigoCvm":row["cvm_code"] or "",
            "preco":fmt_money(row["price"]),"precoRaw":row["price"],
            "variacao":fmt_pct(row["variation"],True),"volume20":fmt_money(row["avg20"]),
            "volume20Raw":row["avg20"],"pregoes20":row["sessions20"],
            "elegivelInicial":row["eligible"],"motivoInicial":row["reason"],
            "filtroQualidade":display,"filtroQualidadeOriginal":original,
            "scoreQualidade":row["quality_score"],"falhas":row["failures"],
            "pendencias":row["pending"],"motivoQualidade":row["quality_reason"] or "DFP/ITR ainda não consolidado.",
            "financeira":bool(row["is_financial"]),"roe5a":fmt_pct(row["roe_avg_5y"]),
            "roe5aRaw":row["roe_avg_5y"],"cagrLucro5a":fmt_pct(row["profit_cagr_5y"]),
            "dlEbitda":"N/A SETOR" if row["is_financial"] else fmt_multiple(row["net_debt_ebitda"]),
            "margemLiquida":fmt_pct(row["net_margin_latest"]),"tendenciaMargem":row["margin_trend"] or "",
            "lucrosPositivos5a":row["positive_profits_5y"],"receitaLtm":fmt_money(row["revenue_ltm"]),
    ...
    payload={
        ...
        "latestFundamentalsDate":max((i["referenciaFundamentos"] for i in items),default=""),
...
def run() -> int:
    ...
    try:
        ...
        issuers=[dict(row) for row in conn.execute("SELECT DISTINCT cnpj,cvm_code,company,segment FROM universe WHERE cnpj<>''")]
        summary=load_and_score_fundamentals(conn,issuers)
        log(conn,"OK",f"Mercado: {changed} ativos, base {quote_day}. Fundamentos: {summary['processed']} emissores; {summary['red_alerts']} alertas.")
