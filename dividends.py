...
def recalc_quality(conn: sqlite3.Connection, cnpj: str, payout: float | None, note: str) -> str:
    ...
    reasons: list[str] = []
    if failures:
        reasons.append(f"{failures} reprovação(ões)")
    if structural_red:
        reasons.append("deterioração estrutural identificada no filtro")
    if financial:
        reasons.append("métricas regulatórias setoriais pendentes")
    if pending:
...
def calculate_metrics(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    for row in principals:
        ...
        elif quality_status == "ALERTA VERMELHO":
            valuation_status = "BLOQUEADO — Alerta Vermelho"
        elif quality_status == "APROVADA PARCIAL — SETOR FINANCEIRO":
            valuation_status = "BLOQUEADO — métricas regulatórias do setor financeiro pendentes"
        elif quality_status != "APROVADA NO FILTRO":
            valuation_status = "BLOQUEADO — checklist de qualidade ainda pendente"
        elif dpa is None or payout is None:
            valuation_status = "BLOQUEADO — DPA ou payout não conciliado"
        elif not price or float(price) <= 0:
            valuation_status = "BLOQUEADO — cotação de fechamento indisponível"
        else:
            bazin_ceiling = dpa / BAZIN_YIELD
            bazin_margin = bazin_ceiling / float(price) - 1
            if dpa == 0:
                bazin_status = "SEM PROVENTOS 12M"
            elif bazin_margin >= 0.10:
                bazin_status = "ATRATIVA PELO MÉTODO"
            elif bazin_margin >= 0:
                bazin_status = "NEUTRA PELO MÉTODO"
            else:
                bazin_status = "CARA PELO MÉTODO"
            valuation_status = "LIBERADO — Bazin calculado com fechamento oficial B3"
        ...
        conn.execute(
            ...
            INSERT INTO dividend_metrics(
              ticker,root,cnpj,quote_date,window_start,dpa_12m,dividend_yield,total_shares,lpa,payout,
...
def event_payload(conn: sqlite3.Connection, ticker: str, start: str, end: str) -> list[dict]:
    rows = conn.execute(
        """
        SELECT label,last_date_prior,payment_date,rate,related_to,remarks
          FROM cash_dividends
         WHERE ticker=? AND eligible_dpa=1
           AND last_date_prior>=? AND last_date_prior<=?
         ORDER BY last_date_prior DESC,payment_date DESC
...
def enrich_site(conn: sqlite3.Connection, summary: dict[str, int], import_summary: dict[str, int]) -> dict:
    ...
    payload["methodology"]["quality"] = (
        ...
        "lucro positivo em ao menos 4 de 5 anos; payout 12M < 90%. Duas reprovações, "
        "prejuízo recorrente ou dívida explosiva geram Alerta Vermelho."
    )
    payload["methodology"]["valuation"] = (
        "Bazin somente após o filtro de qualidade e com DPA/payout conciliados. "
        "DPA 12M usa dividendos e JCP cuja última data COM está entre a data-base de mercado "
        "e os 365 dias anteriores. Preço-teto = DPA 12M / 7,75%; margem = preço-teto / fechamento B3 − 1."
    )
    payload["disclaimer"] = (
